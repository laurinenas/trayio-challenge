function toArray(text) {
	...
	throw new Error(`Given input not compatible with hoover specification. Cooridnates should be a pair of integers, got: ${text}`)
}
...
module.exports = function (text) {
	if (typeof text !== 'string') throw new Error(`Should be given a text according to specs. Got: ${text}`)
	const lines = text
		.split('\n')
		.filter(x => Boolean(x))
